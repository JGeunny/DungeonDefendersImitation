...
void CCameraObj::LateReady_Object(void)
{
	...
	if(pair_Camera.first != pair_Camera.second)
		Set_Target(pair_Camera.first->second);
}
...
{
	if (!Engine::Is_DebugRender(DEBUG_RENDER_CAMERA)) return;
...
{
	m_bPlay = true;
	auto pCamera = Get_Target();
	if (pCamera != nullptr)
		pCamera->Set_Activate(false);
}
